...
main (int argc, char *argv[])
{
  ...
  while (1)
    {
      int n, i;
      /*MW: note that the buffer with triggered file descriptors is returned */
      n = epoll_wait (efd, events, MAXEVENTS, -1);
      for (i = 0; i < n; i++)
	{ 
          /*MW: added a send method */ 
          printf("start of event loop");
          if (index == 2)
          {
            send_default_message (fd_array[2]);
          }
              ...
              while (1)
                {
                  ...
                  //event.events = EPOLLIN | EPOLLET;i
